"""2D distribution plot comparison for the 241 model in machine input units.
...
particles_241 OpenPMD beam distributions.

Usage:
    python compare_2d_distributions_241.py
    python compare_2d_distributions_241.py --dump-csv 241/dump.csv --max-samples 200
    python compare_2d_distributions_241.py --sample-index 42
"""
...
def plot_covariance_scatter_grid(
    ...
    fig.suptitle(
        "True vs Predicted Covariance Elements (Machine-Unit Inputs, 241 Model)",
        fontsize=14,
    )
...
def build_parser():
    p = argparse.ArgumentParser(
        description=(
            "Compare 2D phase-space distributions from the 241 model "
    ...
    p.add_argument(
        "--dump-csv", default="241/dump.csv",
        help="CSV with particles_241 h5 paths and sim input columns (default: 241/dump.csv)",
    )
    p.add_argument(
        "--output-dir", default="compare-2d-241",
        help="Output directory for plots (default: compare-2d-241)",
    )
    p.add_argument(
...
def main():
    ...
    has_h5 = (
        df["particles_241"].notna()
        & df["particles_241"].astype(str).str.endswith(".h5")
    )
    ...
    # ── Ground truth from particles_241 h5 files ──────────────────────────────
    print("[run] Loading ground-truth covariances from particles_241 h5 files ...", flush=True)
    h5_paths = df_valid["particles_241"].tolist()
    true_covs_list = []
    valid_indices = []
