...
def main():
    ...
    if input_space == "sim":
        ...
        #input_df = pd.DataFrame(X_sim, columns=pv_cols, index=df.index)
    ...
    loader1_lume_check = DataLoader(
        TensorDataset(torch.from_numpy(X_norm)), batch_size=args.batch_size
    )
    ...
    with torch.no_grad():
        for (X_batch,) in loader_sim:
            pred_cov_lume = lume_sim_model(X_batch.to(device)).cpu().numpy()
    ...
    if not np.allclose(preds_cov_lume_ref, preds_cov_lume_sim, rtol=1e-5, atol=1e-5):
        max_abs_diff = float(np.max(np.abs(preds_cov_lume_ref - preds_cov_lume_sim)))
    ...
    # in sim->machine->sim conversion (esp. GUNF:rf_field_scale with 7.89e-7 scaling)
    ...
    with torch.no_grad():
        for (X_batch,) in loader_machine:
            pred_cov_lume = lume_machine_model(X_batch.to(device)).cpu().numpy()
    ...
    #print(df[sim_cols].loc[row_index, pv_cols], flush=True)
